...
public:
        ...
        )
    {
        ...
        for (size_t i = 0; i < num_filter; i++) {
            ...
            std::vector<u_int8_t> quant_vals; // bilal
            std::vector<size_t> std_idx_list;
...
#if EIDSP_QUANTIZE_FILTERBANK
                //bial
                std_idx_list.push_back(index);
                quant_vals.push_back(numpy::quantize_zero_one(z.buffer[zx]));
...
#if EIDSP_QUANTIZE_FILTERBANK
            for (int index = 0; i < quant_vals.size(); index++)
            {
                filterbanks->buffer[std_idx_list[index]] = quant_vals[index];
            }
#endif
        }
